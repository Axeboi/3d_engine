    ...
    while(std::getline(obj_file, line))
    {
        ...
        if(line.substr(0, 2) == "f ")
        {
            ...
            while(faces_from_file >> part)
            {
                ...
                triangle.tri[i] = vertex_arr[stoi(token)];
                start = end + 1;
                ...
                triangle.normal = normal_arr[stoi(token)];
...
int main()
{
    ...
    triangle_buffer_init(triangles);

    for (size_t i = 0; i < triangles.size; i++)
    {
        for(size_t v = 0; v < 3; v++)
        {
            // std::cout << triangles.tris[i].tri[v].x << " " << triangles.tris[i].tri[v].y << " " << triangles.tris[i].tri[v].z << " " << std::endl;
            //std::cout << triangles.tris[i].normal.x << " " << triangles.tris[i].normal.y << " " << triangles.tris[i].normal.z << " " << std::endl;
        }
    }
}